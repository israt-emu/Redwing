	...
	useEffect(() => {
		...
		if (data && data.users && data.users.length) {
			data.users.forEach(user => {
				...
				// 	if (project.project_name === 'RedWing') {
				// 		playUsers.push(user);
				// 	} else {
				// 		users.push(user);
				// 	}
				...
				if (user?.completed_todo > 5) {
					moreThanFive.push(user);
				}
				if (user?.completed_todo >= 1 && user?.completed_todo <= 5) {
					oneToFive.push(user);
				}
			...
			// console.log(playUsers);
			if (segment === 'more_than_five') {
				const sorted = [...moreThanFive].sort((a, b) =>
					a.completed_todo < b.completed_todo ? 1 : -1
				);
				setUsers(sorted);
			}
			if (segment === 'one_to_five') {
				const sorted = [...oneToFive].sort((a, b) =>
					a.completed_todo < b.completed_todo ? 1 : -1
				);
				setUsers(sorted);
			}
			if (segment === 'zero') {
				const sorted = [...zero].sort((a, b) => (a.completed_todo < b.completed_todo ? 1 : -1));
				setUsers(sorted);
			}
				...
				<Grid container spacing={2}>
					...
					<Grid item xs={8} sm={10} style={{ fontSize: '14px' }}>
						<a
							...
							style={{
								color:
									props.user_id !== 34188691 && props.active_todo === 0
										? 'red'
										: props.user_id !== 34188691 &&
